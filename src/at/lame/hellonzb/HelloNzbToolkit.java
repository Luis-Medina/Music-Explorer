...
public class HelloNzbToolkit {
    ...
    public static boolean testServerConnection(boolean showSuccess) {
        ...
        String port = (String) Main.prefs.get("usenetport");       
        String hostname = (String) Main.prefs.get("usenethost");
        String timeout = Main.timeout;

        // check for specified hostname
        if (hostname.length() == 0 || port.length() == 0 || timeout.length() == 0) {
            msg = "PopupServerNotSet";
            error = true;
        } else {
            try {
                ...
                if (diff > (Integer.valueOf(timeout) * NANO_MODIFIER)) {
                    msg = "PopupServerConnectionError";
                    error = true;
        ...
        if (error) {
            Main.getLogger().log(Level.SEVERE, null, msg);
            JOptionPane.showMessageDialog(null, msg, "Error connecting!", JOptionPane.INFORMATION_MESSAGE);
            return false;
        } else if (showSuccess) {
    ...
    private static int[] getVersionParts(String ver) {
        ...
        if (tmp.length > 0) {
            parts[0] = Integer.valueOf(tmp[0]);
        }
        if (tmp.length > 1) {
            parts[1] = Integer.valueOf(tmp[1]);
        }
        if (tmp.length > 2) {
            parts[2] = Integer.valueOf(tmp[2]);
        }
        if (tmp.length > 3) {
            parts[3] = Integer.valueOf(tmp[3]);
        }
