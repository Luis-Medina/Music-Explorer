...
public class NettyNioClient implements Runnable
{
			...
			throws IOException, IllegalArgumentException, UnknownHostException
	{
		...
		this.useSSL = (Boolean) Main.prefs.get("usenetssl");
		this.serverAddress = InetAddress.getByName(Main.prefs.get("usenethost").toString());
                /*
		if(this.useSSL)
			...
			this.port = Integer.valueOf(mainApp.getPrefValue("ServerSettingsPort"));
                 * 
                 */
		this.username = Main.prefs.get("usenetuser").toString();
		this.password = Main.prefs.get("usenetpass").toString();
                this.port = Integer.parseInt(Main.prefs.get("usenetport").toString());
		this.threadcount = Integer.parseInt(Main.prefs.get("maxdownloads").toString());
		
		this.ncMgr = new NettyChannelManager();
		...
		channelPipelineFactory = new NettyNioClientPipelineFactory(
				this, ncMgr, username, speedLimit, useSSL); 
	...
	public void run()
	{
		...
		{
			Main.getLogger().log(Level.SEVERE, null, e.getCause());
			runFlag = false;
		}
		...
		Main.getLogger().log(Level.INFO, "NettyNioClient stopped");
	} 
	...
	{
		Main.getLogger().log(Level.INFO, "NioClient should shutdown now");
		...
		for(int i = 0; i < ncMgr.size(); i++)
		{
			...
			if(currTime > timeout || !channel.isOpen())
			{
				try
				{
					...
					String msg = "ThreadViewTimeoutReconnecting";
	...
	private boolean writeStatusCmds(Channel channel, boolean cmdFlag) throws IOException
	{
		...
		if(this.testSegAvailability == false)
		{
			...
			if(cmdFlag == true)
			{
				...
				String msg = "ThreadViewStatusFetchArticle" +
		...
		else
		{
			...
			if(cmdFlag == true)
			{
				...
				else
				{
					...
					// update thread view
					String msg = "ThreadViewStatusCheckArticle" +
							" <" + article + ">";
					updThreadView(channel, msg);
				}
			}
	...
	protected long getSpeedLimit()
	{
		...
		try
		{
			limit = Long.valueOf(Main.prefs.get("speedlimit").toString());
