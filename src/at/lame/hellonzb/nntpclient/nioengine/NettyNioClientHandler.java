...
public class NettyNioClientHandler extends SimpleChannelUpstreamHandler
{
	 ...
	 */
	public NettyNioClientHandler(NettyNioClient client, 
	...
	public void channelConnected(ChannelHandlerContext ctx, ChannelStateEvent e) throws Exception 
	{
		if(useSSL)
		{
			...
			if(!future.isSuccess())
			{
				Main.getLogger().log(Level.SEVERE, "SSL handshake error");
	...
	private void handleResponse(Channel channel, String response, byte [] rspData) 
	{
		...
		switch(status)
		{
			...
			case RECEIVING_DATA:
				...
				// wenn wir nach einem verbindungsabbruch w�hrend RECEIVING_DATA nicht
				// mehr wirklich weitere daten eines artikels bekommen (weil stattdessen
				// neu authentifiziert werden m�sste, und stattdessen eine fehlermeldung
			...
			ChannelStatus status, int handlerErrorCode)
	{
		...
		else
		{
			...
			else if(nettyNioClient.isTestAuthFlag())
			{
				...
				Main.getLogger().log(Level.SEVERE, "no RspHandler found!");
		}
