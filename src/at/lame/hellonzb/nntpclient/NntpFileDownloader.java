...
public class NntpFileDownloader implements Runnable {
     ...
     */
    public NntpFileDownloader(NettyNioClient nioClient,
            SegmentQueue segQueue, File dlDir, HelloNzb mainApp) {
     ...
     */
    public void run() {
        int maxThreads = (Integer) Main.prefs.get("maxdownloads");
        ...
        while (!shutdown && (segQueue.hasMoreSegments() || runningThreads > 0)) {
            // more segments to go?
            while (segQueue.hasMoreSegments() && runningThreads < maxThreads
                    && !pause && nioClient.hasFreeSlot()) {
                ...
                //Main.getLogger().log(Level.INFO, "Downloading next segment of file: {0}", filename);
                ...
                // start data download
                nioClient.fetchArticleData(seg.getGroups().firstElement(),
                        seg.getArticleId(), newHandler);
        ...
        Main.getLogger().log(Level.SEVERE, "FileDownloader has finished downloading all files");
    }
    ...
    private void handleFinishedDlFile(final DownloadFile dlFile) {
        ...
        Main.getLogger().log(Level.INFO, "File downloading finished: " + filename);
        ...
        Main.getLogger().log(Level.INFO, "First line(s) dump:\n{0}", HelloNzbToolkit.firstLineFromByteData(articleData.get(0), 2));

        // determine data encoding
        String encoding = null;
        if (bytesEqualsString(articleData.get(0), "=ybegin")) {
            encoding = "yenc";
        } else if (bytesEqualsString(articleData.get(0), "begin ")) {
            encoding = "uu";
        } else {
            // too bad, no decoder found for this file :(
            Main.getLogger().log(Level.SEVERE, "No suitable decoder found for downloaded file: {0}", dlFile.getFilename());
