...
public class HelloYenc {
    ...
    private boolean checkFooter(byte[] in, int idx) throws YencException {
        ...
        if (in[idx] != 'p' && in[idx] != 'c') {
            ...
            Main.getLogger().log(Level.SEVERE, "no CRC32 value found in yenc footer");
            return true;
        }
        ...
        if (crc32Obj.getValue() != crc32ToLong(crc32)) {
            Main.getLogger().log(Level.SEVERE, "CRC32 check failed in yenc part " + partNum);
            throw new YencException("CRC32 error in yenc part " + partNum);
        } else {
            //Main.getLogger().log(Level.INFO, "yenc CRC32 check ok");
        }
