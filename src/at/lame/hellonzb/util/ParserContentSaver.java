...
public class ParserContentSaver extends Thread
{
	...
	public void run()
	{
		...
		HelloNzbToolkit.emptyDir(new File(Main.getAppDir() + "Data"));
		...
		for(int i = 0; i < parserList.size(); i++)
		{
			...
			if(i == 0)
				success = NzbParser.saveParserData(i, nzbFileName, first);
			else
				success = NzbParser.saveParserData(i, nzbFileName, nzbFile.getFiles());
